...
UNS8 SDOtoLine (CO_Data* d, UNS8 line, UNS32 nbBytes, UNS8* data)
{
...
#ifdef SDO_DYNAMIC_BUFFER_ALLOCATION
    {
        ...
        if ((d->transfers[line].offset + nbBytes) > SDO_MAX_LENGTH_TRANSFER) {
            ...
            lineData = d->trans_writeNetworkDictfers[line].dynamicData;
        }
...
UNS8 getSDOlineOnUse (CO_Data* d, UNS8 CliServNbr, UNS8 whoami, UNS8 *line)
{
    ...
    for (i = 0 ; i < SDO_MAX_SIMULTANEOUS_TRANSFERS ; i++){
        if ( (d->transfers[i].state != SDO_RESET) &&
                ...
                (d->transfers[i].whoami == whoami) ) {
            if (line) *line = i;
            MSG_USER(0x1111,"getSDOLinOnUSer:lin:",i);
            return 0;
        }
    }
    return 0xFF;
...
UNS8 proceedSDO (CO_Data* d, Message *m)
{
    ...
    UNS8 cs = 0;
    UNS8 line = 0;
    UNS32 nbBytes = 0; 		/* received or to be transmited. */
    UNS8 nodeId = 0;  	/* The node Id of the server if client otherwise unused */
    UNS8 CliServNbr= 0 ;
    UNS8 whoami = SDO_UNKNOWN;  /* SDO_SERVER or SDO_CLIENT.*/
    UNS32 errorCode = 0; /* while reading or writing in the local object dictionary.*/
    ...
    UNS16 offset = 0;
    UNS16 lastIndex = 0;
    UNS8 SubCommand = 0;	/* Block transfer only */
    UNS8 SeqNo = 0;         /* Sequence number in block transfer */
    UNS8 AckSeq = 0;        /* Sequence number of last segment that was received successfully */
    UNS8 NbBytesNoData = 0 ; /* Number of bytes that do not contain data in last segment of block transfer */
